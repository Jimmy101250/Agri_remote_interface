#ifndef _ROS_novatel_gps_msgs_Gpgsv_h
#define _ROS_novatel_gps_msgs_Gpgsv_h
...
namespace novatel_gps_msgs
{
  ...
  class Gpgsv : public ros::Msg
  {
    public:
      ...
      typedef novatel_gps_msgs::Satellite _satellites_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(satellites_lengthT > satellites_length)
        this->satellites = (novatel_gps_msgs::Satellite*)realloc(this->satellites, satellites_lengthT * sizeof(novatel_gps_msgs::Satellite));
      satellites_length = satellites_lengthT;
      for( uint32_t i = 0; i < satellites_length; i++){
      offset += this->st_satellites.deserialize(inbuffer + offset);
        memcpy( &(this->satellites[i]), &(this->st_satellites), sizeof(novatel_gps_msgs::Satellite));
      }
     return offset;
    }

    const char * getType(){ return "novatel_gps_msgs/Gpgsv"; };
